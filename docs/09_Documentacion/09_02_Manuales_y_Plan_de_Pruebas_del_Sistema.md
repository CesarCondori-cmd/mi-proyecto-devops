...
**PLAN MAESTRO DE PRUEBAS DE SOFTWARE**
* [1. Alcance y Enfoque del Plan de Pruebas](#1-alcance-y-enfoque-del-plan-de-pruebas)
* [2. Estrategia y Niveles de Pruebas Aplicados](#2-estrategia-y-niveles-de-pruebas-aplicados)
* [3. Matriz Expandida de Casos de Prueba Funcionales (CP-F)](#3-matriz-expandida-de-casos-de-prueba-funcionales-cp-f)
* [4. Matriz de Casos de Prueba No Funcionales (CP-NF)](#4-matriz-de-casos-de-prueba-no-funcionales-cp-nf)
* [5. Criterios de Aceptación y Gestión de Defectos](#5-criterios-de-aceptación-y-gestión-de-defectos)
    * [5.1 Criterio de Aprobación y Certificación (Cierre de Ciclo)](#51-criterio-de-aprobación-y-certificación-cierre-de-ciclo)
    * [5.2 Criterio de Rechazo Inmediato (Criterio de Bloqueo)](#52-criterio-de-rechazo-inmediato-criterio-de-bloqueo)

**APLICACIÓN DEL PLAN MAESTRO DE SOFTWARE**
* [1. Tabla de Pruebas Unitarias](#1-tabla-de-pruebas-unitarias)
* [2. Tabla de Pruebas de Integración](#2-tabla-de-pruebas-de-integración)
* [3. Tabla de Pruebas de Sistema](#3-tabla-de-pruebas-de-sistema)
* [4. Tabla de Pruebas de Aceptación](#4-tabla-de-pruebas-de-aceptación)
...
**Metodología de Gestión:** Docs-as-Code / Ciclo de Vida DevOps (CI/CD)

## **1\. Alcance y Enfoque del Plan de Pruebas**

El objetivo primordial de este documento es servir como **guía e instructivo técnico base** para la programación y ejecución de las pruebas en todos sus niveles (Unitarias, Integración, Sistema y Aceptación). El plan asegura la correcta implementación del flujo de detección en puestos fronterizos de la Dirección General de Migración (DIGEMIG), mitigando riesgos críticos como la pérdida de datos por intermitencia de red, la violación de accesos y la fuga de información sensible.

**Objetivos Específicos:**

1. **Garantizar la exactitud del Motor de Reglas (Riesgo):** Validar que las ponderaciones matemáticas clasifiquen correctamente el nivel de peligro (Bajo, Medio, Alto).  
2. **Asegurar la Resiliencia Operativa (Offline-First):** Verificar que el sistema funcione en condiciones de conectividad nula y sincronice de manera segura al recuperar el acceso a la red.  
3. **Cero Tolerancia a Vulnerabilidades:** Comprobar el blindaje del control de acceso basado en roles (RBAC) y el cifrado de datos de las víctimas.

## **2\. Estrategia y Niveles de Pruebas Aplicados**

Para que este documento sea el pilar del desarrollo técnico, las actividades de testing se estructuran de forma piramidal siguiendo el stack oficial (**Java 17 / Spring Boot 3.x, Angular 17+ y PostgreSQL**):

| Nivel de Prueba | Objetivo Técnico de Inspección | Responsable Directo | Contexto / Entorno de Ejecución |
| :---: | :---: | :---: | :---: |
| **Pruebas Unitarias** | Validar de forma aislada la lógica interna	de clases, métodos y servicios (cálculo de riesgo, formateadores) utilizando Mocks/Mockito para aislar la base de datos o APIs externas. Cobertura mínima: 80%. | Equipo de Programadores | Automatizado dentro del **Pipeline de CI** (ejecución mandatoria en cada Commit o Pull Request). |
| **Pruebas de Integración** | Verificar la comunicación entre módulos del backend, la persistencia real mediante JPA/Hibernate hacia PostgreSQL, y el cumplimiento estricto de los contratos de las APIs REST expuestas. | Programadores/ QA Tester | Entorno Local / Servidor de Desarrollo Integrado. |
| **Pruebas de Sistema** | Evaluar el flujo completo del negocio bajo la modalidad de "Caja Negra" de extremo a extremo (E2E), interactuando directamente desde la interfaz en Angular hasta las capas de datos del servidor. | QA Tester/ Especialista | Entorno de Staging (Pre-producción institucional). |
| **Pruebas de Aceptación (UAT)** | Validar el cumplimiento de las Historias de Usuario e idoneidad operativa en los puestos fronterizos antes de autorizar el paso a producción. | Customer (Isaac Surci)/ Inspectores DIGEMIG | Entorno UAT con datos de prueba controlados	y desensibilizados. |

## **3\. Matriz Expandida de Casos de Prueba Funcionales (CP-F)**

Esta matriz asocia de manera unívoca los Requerimientos Funcionales (**RF**) y las Historias de Usuario (**HU**) con el escenario de prueba, los pasos exactos a seguir y la métrica de aceptación requerida.

| CP-F \-01 | RF-01/ HU-01 | Registro de indicadores mediante formulario digital. | Ingresar al formulario de detección. Seleccionar 3 indicadores físicos (ej. signos de coerción),	2 conductuales (ej. incoherencia en el relato) y 1 documental. Presionar "Confirmar". | Almacenamiento exitoso en la BD y despliegue de confirmación visual en la interfaz web en \< 2 segundos. |
| :---: | :---: | :---: | ----- | :---: |
| **CP-F \-02** | RF-02/ HU-02 | Captura de variables y contingencia de hardware. | Probar captura automática de coordenadas GPS y escaneo OCR del documento. Forzar un fallo de hardware en el escáner y verificar la  habilitación  de los campos de texto alternativos. | Captura e inserción de 5 variables principales y 8 de perfilamiento. El sistema permite el ingreso manual alterno de manera fluida ante fallos del lector. |
| **CP-F \-03** | RF-03/ HU-03 | Clasificación automática del nivel de riesgo. | Ingresar un formulario parametrizado con indicadores ponderados de alta criticidad (ej. menor de edad sin tutores y pasaporte adulterado). Guardar el caso. | El Motor de Riesgo (patrón Strategy) calcula e imprime automáticamente en pantalla el nivel: **Bajo, Medio o Alto** en **\< 1 segundo**. |
| **CP-F \-04** | RF-04/ HU-04 | Envío automático de alertas interinstitucionales. | Guardar un registro cuyo resultado del motor sea clasificado como "Riesgo Medio" o "Riesgo Alto". | Envío automatizado del payload del caso vía API REST a los endpoints de la Policía Nacional y Fiscalía en **\< 5 segundos**, registrando el acuse de recibo. |
| **CP-F \-05** | RF-05/ HU-05 | Control de accesos por roles diferenciados (RBAC). | Iniciar sesión con cuenta de rol **Agente Migratorio** e intentar acceder a URLs de administración o reanudación de casos. Iniciar sesión con rol **Recepcionista** e intentar borrar un registro. | El sistema bloquea las	acciones devolviendo un código HTTP 403 (Forbidden). **Regla de	negocio estricta: El rol Recepcionista tiene restringida la eliminación de datos en cualquier nivel.** |
| **CP-F \-06** | RF-06/ HU-05 | Persistencia inalterable de logs de auditoría. | Realizaruna modificación permitida en un registro desde un perfil de Supervisor. Consultar directamente la tabla de auditoría en la base de datos. | Registro inmediato de	la tupla: UsuarioID, Acción ejecutada, Timestamp, IP origen y datos afectados.	 Los registros son de solo lectura y configurados para retención de	 10 años. |
| **CP-F \-07** | RF-07/ HU-07/ HU-08 | Operación transaccional en Modo Offline y sincronización diferida. | Desconectar la tarjeta de red del navegador	del Agente. Llenar y confirmar un formulario de caso. Reconectar la red y verificar el estado del Sync Manager. | El Frontend almacena el caso localmente de maner a segura (IndexedDB/LocalS torage). Al recuperar conexión, sincroniza automáticamente sin duplicidades. Ante colisiones, prevalece el timestamp	más reciente. |

## **4\. Matriz de Casos de Prueba No Funcionales (CP-NF)**

Enfocados en la resiliencia de la infraestructura, seguridad de la información y cumplimiento de los atributos de calidad definidos en el alcance técnico.

| ID Caso | Tipo de Atributo | Ref. RNF | Escenario de Prueba Técnico | Criterio de Aceptación y Validación |
| :---: | :---: | :---: | :---: | ----- |
| **CP-N F-01** | Seguridad y Acceso | RNF-01 | Interceptar el tráfico de red. Intentar peticiones a la API protegida /api/v1/cases enviando un token JWT	 expirado, alterado o nulo. Probar conexiones bajo protocolo HTTP plano. | El backend interrumpe la conexión y devuelve código HTTP 401 (Unauthorized). Se verifica el rechazo total a solicitudes no cifradas, forzando el uso exclusivo de **HTTPS via HSTS**. |
| **CP-N F-02** | Protección de Datos | RNF-02 | Ejecutar pruebas de penetración automáticas (Inyecciones SQL, XSS) en inputs de formularios. Inspeccionar directamente los registros físicos dentro de las tablas de PostgreSQL. | Las entradas se sanitizan de forma estricta   mediante Spring    Security, anulando código malicioso. Los datos confidenciales de filiación e identidad de presuntas víctimas se constatan  **cifrados en reposo (AES-256)**. |
| **CP-N F-03** | Rendimiento y Carga | RNF-03 | Cargar la base de datos PostgreSQL de manera sintética con una volumetría activa de 10,000 registros de casos. Ejecutar ráfagas concurrentes de consultas complejas de filtrado multivariable. | Las consultas aprovechan correctamente los índices relacionales la    BD, completando las peticiones y la transferencia en **\< 1 segundo**, manteniendo estables los consumos de CPU y memoria. |
| **CP-N F-04** | Mantenibilidad | RNF-04 | Someter el repositorio de código fuente a herramientas de análisis estático (ej. SonarQube) para auditar la deuda técnica y el desacoplamiento de la arquitectura. | El código se rige por patrones limpios (Clean Code). Se comprueba	que modificaciones directas sobre el Motor de Reglas (patrón Strategy) no generan	fallos colaterales ni acoplamientos en la capa de persistencia o presentación. |
| **CP-N F-05** | Soporte y Cierre | RNF-05 | Auditoría física y digital del repositorio institucional de entregables para la transferencia tecnológica a DIGEMIG. | Verificación de la disponibilidad completa de la documentación técnica estructurada bajo Docs-as-Code: Manual de Usuario, Manual Técnico de Arquitectura, Plan de Contingencia y código base comentado. |
| **CP-N F-06** | Infraestructura | RNF-06 | Monitorear el comportamiento del despliegue del software en los servidores institucionales. Simular cortes parciales de conectividad con el servidor Nginx y caídas del pool de conexiones. | El sistema se despliega de forma correcta dentro del Data Center de DIGEMIG, demostrando estabilidad y una tasa de disponibilidad operativa mínima comprobada del **99%**. |

## **5\. Criterios de Aceptación y Gestión de Defectos**

### **5.1 Criterio de Aprobación y Certificación (Cierre de Ciclo)**

Para que una fase de pruebas o una versión del sistema se declare **Aprobada** y apta para pasar a Producción, se debe validar de forma cuantitativa lo siguiente:

* El 100% de los Casos de Prueba con prioridad "Must" (Críticos/Bloqueantes) detallados en este plan deben estar ejecutados con estado exitoso (Verde).  
* Los tiempos de respuesta obtenidos en entornos de estrés deben respetar estrictamente los umbrales fijados (\< 1s en consultas complejas, \< 2s en formularios).  
* Cero defectos abiertos con severidad Crítica o Alta en el gestor de incidencia

### **5.2 Criterio de Rechazo Inmediato (Criterio de Bloqueo)**

Se detendrá inmediatamente la ejecución del plan de pruebas, rechazando la versión del software y bloqueando el pipeline automatizado de despliegue (CI/CD) ante cualquiera de los siguientes incidentes:

1. **Fallas de Seguridad Críticas (RNF-01/RNF-02):** Vulnerabilidades que permitan evadir la autenticación de tokens JWT, suplantar roles administrativos o visualizar datos sensibles de víctimas sin el cifrado AES-256 en reposo.  
2. **Corrupción o Pérdida de Datos (RF-07):** Que el Sync Manager descarte, duplique o altere tramas de información de casos guardados en el almacenamiento local (IndexedDB) durante el proceso de sincronización automática al recuperar la conexión a internet.  
3. **Inestabilidad Crítica (RNF-06):** Caídas repetitivas del servidor de aplicaciones que degraden la disponibilidad por debajo del umbral del 99%.

# **APLICACIÓN DEL PLAN MAESTRO DE SOFTWARE**

## **1\. Tabla de Pruebas Unitarias**

**Objetivo:** Validar de forma aislada la lógica interna de la unidad mínima de código sin dependencias externas.

**Entorno de Ejecución:** Entorno local (IDE) y automatizado dentro del Pipeline de CI/CD en cada Commit o Pull Request.

| ID Caso | Archivo / Componente Real de tu Sistema | Escenario de Prueba Técnico | Procedimiento de Verificación y Datos de Entrada | Criterio de Aceptación Métrico (Éxito) | Estado |
| :---: | :---: | :---: | :---: | :---: | :---: |
| **T-UT-01** | backend/src/main/java/.../ services/RiskEvaluationService.java (HU-03 / RF-03) | Verificación de la ponderación matemática del Motor de Riesgo bajo el patrón Strategy. | Inyectar un objeto simulado (Mock) en la función que contenga indicadores críticos de la guía oficial: minoría de edad sin acompañamiento legal y pasaporte adulterado. | El método calculateRiskLevel() debe retornar estrictamente el enum RiskLevel.ALTO con un tiempo de cálculo \< 1 segundo. | **APROBADO** |

| T-UT-02 | backend/src/main/java/.../ services/CaseManagement Service.java (HU-05 / RF-05) | Restricción estricta de permisos de eliminación para el rol Recepcionista. | Simular el contexto de seguridad mediante la anotación @WithMockUser(role s={"RECEPCIONIST A"}) e invocar el método de negocio deleteCase(id). | La ejecución debe disparar de forma obligatoria una excepción de tipo AccessDeniedExcepti on. El método .delete() del repositorio nunca debe ejecutarse. | APROBADO |
| :---: | :---: | :---: | :---: | :---: | :---: |
| **T-UT-03** | frontend-nuevo/src/app/ca sos/services/offline-storag e.service.ts (HU-07 / RF-07) | Almacenamiento temporal en la memoria interna del navegador web del Agente. | Mockear el objeto transaccional global window.indexedDB del navegador e invocar la función de persistencia local saveDraft(casePayloa d). | Verificación de la inserción correcta de la trama estructurada JSON dentro del almacén local de datos del cliente mediante una promesa resuelta con éxito. | **APROBADO** |

## **2\. Tabla de Pruebas de Integración**

**Objetivo:** Verificar la comunicación entre módulos, persistencia en base de datos y cumplimiento de contratos de APIs.

**Entorno de Ejecución:** Servidor de Desarrollo Integrado, entorno local o contenedores de bases de datos relacionales.

| ID Caso | Archivos / Componentes Reales de tu Sistema | Escenario de  Prueba Técnico | Procedimiento de Verificación y Datos de Entrada | Criterio de Aceptación Métrico (Éxito) | Estado |
| :---: | :---: | :---: | :---: | :---: | :---: |
| **T-IT-01** | backend/.../controllers/CaseC ontroller.java $\\rightarrow$ **PostgreSQL**  (HU-01 / RF-01) | Persistencia real del Formulario Digital de Detección en las tablas físicas de la base de datos. | Lanzar una petición HTTP POST simulada mediante el uso de MockMvc al endpoint expuesto /api/v1/cases, enviando la estructura JSON de los indicadores. | El servidor debe responder con un código de estado 201 Created. Los datos deben guardarse físicamente en PostgreSQL en un tiempo \< 2 segundos. | **APROBADO** |

| T-IT-02 | backend/src/main/java/.../syn c/SyncManager.java (HU-07 / RF-07) | Sincronización automática de datos diferidos desde la cola offline del cliente. | Enviar un lote de 3 casos con marcas de tiempo (timestamps) manuales que simulen provenir de la persistencia local del cliente tras una reconexión. | El backend procesa e inserta los 3 registros secuencialmente en PostgreSQL sin presentar pérdida de información ni duplicidad de registros. | APROBADO |
| :---: | :---: | :---: | :---: | :---: | :---: |
| **T-IT-03** | backend/.../integration/Police FiscaliaFacade.java (HU-04 / RF-04) | Interoperabilidad y consumo correcto de los endpoints externos de Policía y Fiscalía. | Guardar un registro transaccional en el sistema cuyo resultado del motor de reglas sea clasificado con un nivel de "Riesgo Medio" o "Riesgo Alto". | El cliente HTTP integrado de Java debe estructurar el payload y enviarlo con éxito vía API REST a los endpoints externos correspondientes en \< 5 segundos con acuse de recibo. | **APROBADO** |

## **3\. Tabla de Pruebas de Sistema**

**Objetivo:** Evaluar el flujo completo del negocio bajo la modalidad de "Caja Negra" interactuando desde la interfaz hasta las capas de datos.

**Entorno de Ejecución:** Entorno o servidor de Staging (Pre-producción que replica la infraestructura de TI institucional).

| ID Caso | Componentes e Interfaces Reales de tu Sistema | Escenario de Prueba Técnico | Procedimiento de Verificación y Datos de Entrada | Criterio de Aceptación Métrico (Éxito) | Estado |
| :---: | ----- | :---: | ----- | :---: | :---: |
| **T-ST-01** | frontend-nuevo/src/app/au th/pages/login.component .ts frontend-nuevo/src/app/ca sos/deteccion/deteccion.c omponent. (HU-07 / RF-07) | Flujo completo de contingencia Offline y posterior proceso de sincronización. | Iniciar sesión en la interfaz y forzar la desconexión total de red de la terminal fronteriza. Llenar y guardar un formulario completo desde deteccion.componen t.html. Restablecer la conectividad de la red de la computadora. | La aplicación web resguarda el caso localmente de manera segura sin mostrar errores en pantalla. Al recuperar la red, el software sube los datos de forma transparente y sin pérdidas al servidor. | **APROBADO** |
| **T-ST-02** | frontend-nuevo/src/app/ca sos/seguimiento/seguimie nto.component.ts backend/.../controllers/Ca seController.java (HU-01, HU-03, HU-04, HU-08) | Ciclo de vida completo del caso: detección, derivación automática y seguimiento interinstitucional. | Registrar un caso con indicadores de peligro crítico como Agente Migratorio desde la interfaz. Validar el disparo de la alerta generada por el backend. Iniciar sesión como rol externo (Policía/Fiscalía) y actualizar el estado a "Judicializado".  | Toda la secuencia transaccional debe completarse sin excepciones en el servidor. Las actualizaciones del estado del caso por externos deben reflejarse en el historial en un tiempo \< 2 segundos. | **APROBADO** |

## **4\. Tabla de Pruebas de Aceptación**

**Objetivo:** Validación final del negocio y confirmación del cumplimiento de las Historias de Usuario antes de autorizar el paso a producción.

**Entorno de Ejecución:** Entorno UAT con datos de prueba controlados, ejecutado por inspectores migratorios y el Customer (Isaac Surci).

| ID Caso | Vistas del Frontend e Infraestructura Real | Escenario de PruebaTécnico | Procedimiento de Verificación y Datos de Entrada | Criterio de Aceptación Métrico (Éxito) | Estado |
| :---: | ----- | :---: | :---: | :---: | :---: |
| **T-A T-01** | frontend-nuevo/src/app/cas os/deteccion/deteccion.com ponent.html (HU-01 / HU-02) | Validación de usabilidad y velocidad en el registro migratorio fronterizo físico. | Un inspector migratorio utiliza una terminal física para cargar un caso simulado, forzando fallas de hardware en el escáner para evaluar la respuesta de la interfaz ante la inserción manual alternada. | El usuario final manifiesta conformidad con la simplicidad de la interfaz. Los datos se capturan y los mensajes informativos de confirmación visual toman \< 2 segundos en pantalla. | **APROBADO** |
| **T-A T-02** | frontend-nuevo/src/app/cas os/seguimiento/seguimient o.component.html (HU-12 / RF-12) | Validación del control estricto de seguridad para la reapertura de casos cerrados. | Un usuario con rol de Supervisor intenta cambiar el estado de un caso "Cerrado" a "Reabierto" dejando el campo de justificación vacío en el modal de la interfaz, y posteriormente repite el proceso escribiendo una justificación formal. | El sistema bloquea el guardado si el campo está vacío. Al ingresar el texto, habilita la reapertura, cambia el estado de la tabla e inyecta la justificación en los logs de auditoría. | **APROBADO** |
| **T-A T-03** | backend/src/main/resources /backup-script.sh **Pipeline CI/CD (GitHub Actions / GitLab CI)**  (HU-13 / HU-14) | Validación operativa de la ejecución de respaldos automáticos y rollback del software. |  |  |  |
